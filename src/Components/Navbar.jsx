...
//   return (
//     <nav className="fixed bg-white shadow-md w-full z-50 h-[90px]">
...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 z-50 h-[90px] w-full bg-white shadow-md">
      <div className="container mx-auto flex items-center justify-between px-3">
        {/* Logo */}
        <figure className="w-24">
          <a href="#home">
        ...
        <ul className="hidden flex-row gap-6 px-5 py-3 md:flex">
          {navLinks.map((link, index) => (
            <li key={index} onClick={() => setActiveLink(link.href)}>
              <a
                href={link.href}
                className={`text-lg font-semibold transition hover:text-blue-600 ${
